 ...
 * \return					number of errors
 */
uint16_t create_entry_file(const char * file_name, link_object_t * link_table,
                           uint16_t link_table_len) {
    char * file_name_no_ext = get_file_name_no_ext(file_name);
    FILE * fp = NULL;
    uint16_t i;
    uint16_t errors = 0;
    ...
    char * entry_name =
        (char *)malloc(strlen(file_name_no_ext) + 4 + 1); /* ".ent" + NULL */
    if (entry_name) {
 ...
 * \return					number of errors
 */
uint16_t create_extern_file(const char * file_name,
    ...
    char * extern_name =
        (char *)malloc(strlen(file_name_no_ext) + 4 + 1); /* ".ext" + NULL */
    if (extern_name) {
